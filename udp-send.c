...
//---------------------------------------------------------------------------------//
	...
	int fd, fd1, fd2, readcnt, i=1, slen=sizeof(remaddr), bsize, rn, tot;
...
//---------------------------------------------------------------------------------//
	...
	while(1){
		...
		if((readcnt = read(fd, d_buf, DBUFLEN))==0){
      	...
      	//send message
      	if (sendto(fd2, d_buf, sizeof(d_buf), MSG_DONTWAIT, (struct sockaddr *)&remaddr, slen)==-1){
      		++f_count;
